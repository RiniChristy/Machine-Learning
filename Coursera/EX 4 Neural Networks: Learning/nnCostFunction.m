...
a1 = [ones(m,1),X]; % Add a column of ones to x; a1: 5000x401 

z2 = a1 * Theta1'; % 25 units in the second layer; z2: 5000x25 
a2 = sigmoid(z2); 
m = size(a2, 1);
a2= [ones(m,1),a2]; % Add a column of ones to a2; a2: 5000x26
z3 = a2 * Theta2';
...
%
d3= a3 - y_matrix;
d2 = d3* (Theta2).*[ones(size(z2,1),1) sigmoidGradient(z2)]; % The size is (m x r)⋅(r x h) --> (m x h). 
D1 = d2(:,2:end)' * a1; % The size is (h x m)⋅(m x n) --> (h x n)
D2 =  d3' * a2; % The size is (r x m)⋅(m x [h+1]) --> (r x [h+1])
